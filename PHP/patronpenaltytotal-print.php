...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <div class ="row">
    <div class="col-8">
    <h3 class="mb-4 mt-3 text-uppercase">Total Summary of Penalties Per Patron<button class="select btn btn-primary" onClick="window.print()"><a class="nav-link" href="#">Print</a></button></h3>
    </div>
    <div class="col-4">
        <?php
    ...
    <form method="POST" action="patronpenaltytotal-print.php" class="form">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['filter-type'])) {
            $filterType = $_POST['filter-type'];
            $query = "SELECT count(rb.patronID) AS count, pt_name AS name, sum(penalty_paid) AS penalty 
            FROM returned_book rb 
            INNER JOIN patron_acc pa 
            ON rb.patronID = pa.patronID 
            WHERE penalty_paid > 0 ";

            if ($filterType == 'none') {
                $query .= "";
            } elseif ($filterType == 'week') {
                $query .= " && YEARWEEK(return_date, 1) = YEARWEEK(CURDATE(), 1)";
            } elseif ($filterType == 'month') {
                $query .= " && YEAR(return_date) = YEAR(CURDATE()) AND MONTH(return_date) = MONTH(CURDATE())";
            } elseif ($filterType == 'custom') {
                if (isset($_POST['start-date']) && isset($_POST['end-date'])) {
                    $startDate = $_POST['start-date'];
                    $endDate = $_POST['end-date'];
                    if ($startDate && $endDate) {
                        $query .= " && return_date BETWEEN '$startDate' AND '$endDate'";
                    } else {
