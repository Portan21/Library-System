...
<head>
  ...
  <title>REPORTS | Total Book Return Approval Of Librarian</title>
...
<body class="hold-transition sidebar-mini layout-fixed">
    ...
    <div class ="row">
    <div class="col-8">
    <h3 class="mb-4 mt-3 text-uppercase">Total Count of Book Return Approval<button class="select btn btn-primary" onClick="window.print()"><a class="nav-link" href="#">Print</a></button></h3>
    </div>
    <div class="col-4">
        <?php
    ...
    <form method="POST" action="totalreturn-print.php" class="form">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['filter-type'])) {
            $filterType = $_POST['filter-type'];
            $query = "SELECT count(rb.librarianID) as total, name 
            FROM returned_book rb
            INNER JOIN lib_acc lc
            ON rb.librarianID = lc.librarianID";
            ...
            if ($result->num_rows > 0) {
                echo "<table id='example' class='content-table' style='width:100%'>
                      <thead>
                          <tr>
                          <th class='px-4 py-2 text-center'>Total Return Approval</th>
